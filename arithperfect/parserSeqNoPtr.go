...
func parseStringNoPtr(data []byte) (int64, error) {
	startLex := 0
	endLex := 0
	rule := 0
	...
	for startLex < len(data) {
		...
		controlState, symbol, err = lexerExecutorNoPtr(rule, startLex, endLex, data, memoryStack, &dmemTop)
		if err != nil {
			return -1, err
		}
		if controlState == _LEX_CORRECT {
			...
			for precedence == _TAKES_PREC {
				...
				parserExecutorNoPtr(parsingRule, memoryStack, &dmemTop)
